...
async def check_domains_and_send_alerts(
    ...
    async for result in get_cert_expiration_many(domains):
        ...
        should_alert = (
            data.is_expired or days_remaining <= 0 or days_remaining in warning_days
        )
        if should_alert:
...
Examples:
  ...
  python check_cert_email.py --config config.ini --dry-run google.com
    ...
    parser.add_argument(
        "domains", nargs="+", help="One or more domain names to check"
    )
    ...
    try:
        asyncio.run(
            check_domains_and_send_alerts(
                domains, email_from, email_to, warning_days, args.dry_run
            )
        )
